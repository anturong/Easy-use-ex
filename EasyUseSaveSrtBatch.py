...
class EasyUseSaveSrtBatch:
    ...
    def save_srt(
        ...
        if not SRT内容 or not SRT内容.strip():
            print("[Easy Use Ex] Error: No SRT content provided for save.")
            return {"ui": {}, "result": (SRT内容, "")}

        stem = build_combined_stem(
            文件名前缀,
            strip_known_extension(文件名, ".srt"),
            文件名分隔符,
            "subtitle",
        )
        ...
        try:
            with open(output_file, "w", encoding="utf-8-sig", newline="\n") as file:
                file.write(SRT内容.strip() + "\n")
            print(f"[Easy Use Ex] SRT saved to: {output_file}")
        except Exception as exc:
            print(f"[Easy Use Ex] Error saving SRT: {exc}")
            return {"ui": {}, "result": (SRT内容, "")}
        ...
        return {"ui": ui, "result": (SRT内容, output_file)}
